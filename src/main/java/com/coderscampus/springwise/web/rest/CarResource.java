...
/**
 * REST controller for managing {@link com.coderscampus.springwise.domain.Car}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CarResource {
    ...
    public ResponseEntity<Car> partialUpdateCar(@PathVariable(value = "id", required = false) final Long id, @RequestBody Car car)
        ...
        Optional<Car> result = carRepository
            .findById(car.getId())
            .map(existingCar -> {
                if (car.getMotorSize() != null) {
                    existingCar.setMotorSize(car.getMotorSize());
                }
                if (car.getModelName() != null) {
                    existingCar.setModelName(car.getModelName());
                }
                if (car.getWheelSize() != null) {
                    existingCar.setWheelSize(car.getWheelSize());
                }
                ...
                if (car.getYearOf() != null) {
                    existingCar.setYearOf(car.getYearOf());
                }
