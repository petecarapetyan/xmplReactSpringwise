  ...
  return (
    <div>
      <h2 id="ticket-heading" data-cy="TicketHeading">
        <Translate contentKey="xmplReactSpringwiseApp.ticket.home.title">Tickets</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="xmplReactSpringwiseApp.ticket.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/ticket/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="xmplReactSpringwiseApp.ticket.home.createLabel">Create new Ticket</Translate>
          </Link>
        </div>
      </h2>
        ...
        {ticketList && ticketList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="xmplReactSpringwiseApp.ticket.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {ticketList.map((ticket, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{ticket.issue}</td>
                  <td>{ticket.timeStamp ? <TextFormat type="date" value={ticket.timeStamp} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{ticket.student ? <Link to={`/student/${ticket.student.id}`}>{ticket.student.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="xmplReactSpringwiseApp.ticket.home.notFound">No Tickets found</Translate>
            </div>
          )
        )}
      </div>
